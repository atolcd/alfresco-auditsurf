...
public class LastModifiedDocumentsGet extends DeclarativeWebScript implements InitializingBean {
	...
	protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
		try {
			...
			List<Map<String, Object>> entries = auditQueriesService.lastModif();
			Map<Date, NodeRef> lastmodifiedList = new HashMap<Date, NodeRef>();

			for (Map<String, Object> entry : entries) {
				NodeRef node = (NodeRef) entry.get("value");

				if (this.nodeService.exists(node)) {
					lastmodifiedList.put((Date) entry.get("date"), node);
				}
			}

			// Insert into the map
			model.put("LastModifiedDocs", lastmodifiedList.entrySet());

			return model;
		} catch (Exception e) {
			throw new WebScriptException("[LastModifiedDocumentsGet] Error in executeImpl function");
		}
	}
}